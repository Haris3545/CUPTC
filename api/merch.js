...
import { isMember, isCommittee, readJSON, writeJSON, storageReady, send, body } from './_lib/core.js';
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      const out = items.slice().sort((a, b) => (a.order || 0) - (b.order || 0))
        .map((m) => (member ? m : { id: m.id, name: m.name, description: m.description, image: m.image, order: m.order }));
    ...
    if (req.method === 'POST') {
      ...
      const item = {
        ...
        delivery: clean(i.delivery, 60), sizes: clean(i.sizes, 80), image: clean(i.image, 60),
